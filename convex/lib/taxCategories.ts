...
export function isValidTaxCategory(
	section: TaxSection,
	category: string,
): boolean {
	return (TAX_CATEGORIES[section] as readonly string[]).includes(category);
}

export function assertTaxCategory(section: TaxSection, category: string): void {
	if (!isValidTaxCategory(section, category)) {
		throw new Error("TAX_CATEGORY_INVALID");
	}
}
